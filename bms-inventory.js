import { CheerioCrawler, log } from 'crawlee';
import puppeteer from 'puppeteer';
import * as cheerio from 'cheerio';
import mysql from 'mysql2/promise';
import crypto from 'crypto';
import { parseArgs } from 'util';

// --- 1. PARAMETER PARSING ---
const { values } = parseArgs({
    options: {
        url: { type: 'string', default: 'https://batterymegastore.b2bwave.com' },
        b_email: { type: 'string' },
        b_pass: { type: 'string' }, // We use the real password here for Puppeteer login
        db_host: { type: 'string' },
        db_user: { type: 'string' },
        db_pass: { type: 'string' },
        db_name: { type: 'string' },
        db_table: { type: 'string' },
    },
    strict: false
});

const baseUrl = values.url.replace(/\/$/, '');

// --- 2. PUPPETEER LOGIN (GET COOKIES) ---
async function getSessionCookies() {
    console.log('Launching hidden browser for B2BWave login...');
    const browser = await puppeteer.launch({ headless: 'new', args: ['--no-sandbox'] });
    const page = await browser.newPage();
    
    await page.goto(`${baseUrl}/customers/sign_in`, { waitUntil: 'networkidle2' });

    // Try standard B2BWave login selectors
    await page.type('#user_email, input[type="email"]', values.b_email);
    await page.type('#user_password, input[type="password"]', values.b_pass);
    
    console.log('Submitting login form...');
    await Promise.all([
        page.waitForNavigation({ waitUntil: 'networkidle2' }),
        page.click('input[type="submit"], button[type="submit"], .btn-primary')
    ]);

    const cookies = await page.cookies();
    await browser.close();

    // Convert Puppeteer cookies into a standard HTTP Cookie header string
    const cookieString = cookies.map(c => `${c.name}=${c.value}`).join('; ');
    console.log('Login successful! Session cookies retrieved.');
    
    return cookieString;
}

// --- 3. BRAND MAPPING ---
async function getBrandDictionary(cookieString) {
    console.log('Fetching brand category dictionary...');
    const response = await fetch(`${baseUrl}/products/list?category=7`, {
        headers: { 'Cookie': cookieString }
    });
    
    const html = await response.text();
    const $ = cheerio.load(html);
    const dictionary = new Map();

    // Using the class from your screenshot
    $('.card-product-title').each((i, el) => {
        const brandName = $(el).text().trim();
        const href = $(el).attr('href');
        if (href) {
            dictionary.set(brandName.toLowerCase(), baseUrl + href);
        }
    });

    console.log(`Found ${dictionary.size} brands in the B2BWave directory.`);
    return dictionary;
}

// --- 4. MAIN EXECUTION ---
async function main() {
    let db;
    try {
        console.log(`Connecting to database ${values.db_name}...`);
        db = await mysql.createConnection({
            host: values.db_host,
            user: values.db_user,
            password: values.db_pass,
            database: values.db_name
        });

        // Get Input Data
        console.log('Fetching in-scope brands from supplier_partno_prefix...');
        const [rows] = await db.query(`SELECT manufacturer FROM supplier_partno_prefix WHERE Supplier = 'BMS' AND In_scope = 1`);
        const targetBrands = rows.map(r => r.manufacturer);
        console.log(`Found ${targetBrands.length} in-scope brands to scrape.`);

        if (targetBrands.length === 0) return;

        // Recreate output table (Added BrandName column)
        await db.execute(`DROP TABLE IF EXISTS \`${values.db_table}\``);
        await db.execute(`
            CREATE TABLE \`${values.db_table}\` (
                \`API_Vis_Product_List ID\` CHAR(36) PRIMARY KEY,
                \`BrandName\` VARCHAR(255),
                \`Sku\` VARCHAR(255),
                \`Status\` VARCHAR(50),
                \`Price\` DECIMAL(10,2),
                \`Name\` TEXT,
                \`Only_x_left_in_stock\` DECIMAL(10,2)
            )
        `);

        // Get Auth & Dictionary
        const cookieHeader = await getSessionCookies();
        const brandDict = await getBrandDictionary(cookieHeader);

        // Prepare Start Requests
        const startRequests = targetBrands.map(brand => {
            const normalizedBrand = brand.toLowerCase();
            let startUrl;

            if (brandDict.has(normalizedBrand)) {
                // Use Category URL
                startUrl = `${brandDict.get(normalizedBrand)}&per_page=96`;
            } else {
                // Fallback directly to Search URL
                startUrl = `${baseUrl}/products/search_list?utf8=%E2%9C%93&search=${encodeURIComponent(brand)}&per_page=96`;
            }

            return {
                url: startUrl,
                userData: { brandName: brand, isSearchFallback: false }
            };
        });

        // Buffer for database batching
        let productBuffer = [];
        let totalScraped = 0;

        async function flushBufferToDb() {
            if (productBuffer.length === 0) return;
            
            const insertQuery = `
                INSERT INTO \`${values.db_table}\` 
                (\`API_Vis_Product_List ID\`, \`BrandName\`, \`Sku\`, \`Status\`, \`Price\`, \`Name\`, \`Only_x_left_in_stock\`) 
                VALUES ?
            `;
            await db.query(insertQuery, [productBuffer]);
            totalScraped += productBuffer.length;
            productBuffer = []; // Reset buffer
        }

        // --- 5. THE CRAWLER ---
        const crawler = new CheerioCrawler({
            // Pass cookies to every request
            preNavigationHooks: [
                (crawlingContext) => {
                    crawlingContext.request.headers = { 'Cookie': cookieHeader };
                }
            ],
            
            async requestHandler({ $, request, enqueueLinks }) {
                log.info(`Processing: ${request.url}`);
                const { brandName, isSearchFallback } = request.userData;

                // EDGE CASE: Empty Category Page (Jinko Solar fix)
                if ($('.alert-danger').text().includes('no products available')) {
                    if (!isSearchFallback) {
                        log.warning(`Empty category for ${brandName}. Falling back to search...`);
                        const searchUrl = `${baseUrl}/products/search_list?utf8=%E2%9C%93&search=${encodeURIComponent(brandName)}&per_page=96`;
                        await enqueueLinks({
                            urls: [searchUrl],
                            userData: { brandName, isSearchFallback: true }
                        });
                    } else {
                        log.warning(`Search fallback also empty for ${brandName}. Skipping.`);
                    }
                    return; // Stop processing this empty page
                }

                // DATA EXTRACTION
                // NOTE: Inspect B2BWave HTML to confirm these selectors if they return empty.
                // Standard B2BWave product cards usually have a container class like .card-product or .product-item
                $('.card-product, .product-item').each((i, el) => {
                    const title = $(el).find('.card-product-title, .product-title').text().trim();
                    // Product codes are often prefixed with "Code: " or "SKU: "
                    const rawCode = $(el).find('.product-code, .sku').text().replace(/Code:|SKU:/i, '').trim();
                    // Clean price string of currency symbols
                    const rawPrice = $(el).find('.price').text().replace(/[^0-9.]/g, '');
                    // Qty is often in an input box or text
                    const qtyInput = $(el).find('input[name="quantity"]').attr('max');
                    const qtyText = $(el).find('.in-stock, .availability').text().replace(/[^0-9.]/g, '');
                    
                    const qty = parseFloat(qtyInput || qtyText || 0);
                    const status = qty > 0 ? 'IN_STOCK' : 'OUT_OF_STOCK';

                    if (rawCode && title) {
                        productBuffer.push([
                            crypto.randomUUID(),
                            brandName,
                            rawCode,
                            status,
                            parseFloat(rawPrice || 0),
                            title,
                            qty
                        ]);
                    }
                });

                // Batch insert every 100 products to save RAM
                if (productBuffer.length >= 100) {
                    await flushBufferToDb();
                }

                // PAGINATION
                // Look for the "Next" button in the pagination block
                await enqueueLinks({
                    selector: '.pagination a[rel="next"], .next_page a',
                    userData: { brandName, isSearchFallback } // Pass the brand context to the next page
                });
            },
            
            failedRequestHandler({ request }) {
                log.error(`Request ${request.url} failed too many times.`);
            },
        });

        // Start the crawl
        console.log('Starting CheerioCrawler...');
        await crawler.run(startRequests);
        
        // Final flush of any remaining items in the buffer
        await flushBufferToDb();

        console.log(`\n✅ Scrape complete! Saved ${totalScraped} products to ${values.db_table}.`);

    } catch (error) {
        console.error('Script failed:', error);
    } finally {
        if (db) await db.end();
    }
}

main();